...
Example: Enter `1 4 7` to use Anthropic + Gemini + Brave Search.
...
| File | Contents |
|------|----------|
| `secrets/<instance>/gateway.env` | Channel tokens (Discord/Telegram/Slack), AI provider keys (Anthropic, OpenAI, Gemini, etc.), Brave Search API key, Gateway token |
...
8. Tool execution runs in isolated containers when sandbox mode is enabled
